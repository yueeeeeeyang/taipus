-- 创建权限模块首版表。
-- PostgreSQL 使用 partial unique index 表达未删除数据唯一性，保持与 MySQL generated column 方案语义一致。
...
CREATE UNIQUE INDEX uk_sys_permission_applications_active_code ON sys_permission_applications (COALESCE(tenant_id, 'platform'), app_code) WHERE deleted = FALSE;
...
CREATE UNIQUE INDEX uk_sys_permission_menus_active_code ON sys_permission_menus (COALESCE(tenant_id, 'platform'), menu_code) WHERE deleted = FALSE;
...
CREATE UNIQUE INDEX uk_sys_permission_buttons_active_code ON sys_permission_buttons (COALESCE(tenant_id, 'platform'), button_code) WHERE deleted = FALSE;
...
CREATE UNIQUE INDEX uk_sys_permission_apis_active_code ON sys_permission_apis (COALESCE(tenant_id, 'platform'), api_code) WHERE deleted = FALSE;
CREATE UNIQUE INDEX uk_sys_permission_apis_active_route ON sys_permission_apis (http_method, normalized_path) WHERE deleted = FALSE AND status = 'enabled';
...
CREATE UNIQUE INDEX uk_sys_roles_active_code ON sys_roles (tenant_id, role_code) WHERE deleted = FALSE;
...
CREATE UNIQUE INDEX uk_sys_role_relations_active_pair ON sys_role_relations (tenant_id, parent_role_id, child_role_id) WHERE deleted = FALSE;
...
CREATE UNIQUE INDEX uk_sys_role_closures_active_pair ON sys_role_closures (tenant_id, ancestor_role_id, descendant_role_id) WHERE deleted = FALSE;
...
CREATE UNIQUE INDEX uk_sys_account_roles_active_pair ON sys_account_roles (tenant_id, account_id, role_id) WHERE deleted = FALSE;
...
CREATE UNIQUE INDEX uk_sys_permission_grants_active_key ON sys_permission_grants (tenant_id, subject_type, subject_id, resource_type, resource_id, action) WHERE deleted = FALSE;
...
CREATE UNIQUE INDEX uk_sys_permission_versions_active_tenant ON sys_permission_versions (tenant_id) WHERE deleted = FALSE;
